...
func _unhandled_input(event: InputEvent) -> void:
    if event is InputEventKey and event.pressed and not event.echo:
        if event.keycode == KEY_ESCAPE:
            if state == STATE_RUN: _toggle_pause()
            elif state == STATE_SETTINGS: _leave_settings()
...
func _clear_menu() -> void:
    for child in menu_box.get_children(): child.queue_free()
...
func _show_hub() -> void:
    ...
    info_label.text = "Root Amber: %d • Vitality +%d • Damage +%d • Best Depth %d • Wins %d" % [int(data.root_amber), int(data.max_hp_bonus), int(data.damage_bonus), int(data.best_depth), int(data.wins)]
...
func _show_weapons() -> void:
    ...
    for item in weapons:
        ...
        _button("%s — %s" % [item.name, item.desc], _choose_weapon.bind(copy))
...
func _show_marks() -> void:
    ...
    info_label.text = "Weapon: %s. Pick one blessing/curse for this expedition." % selected_weapon.get("name", "Unknown")
    for mark in marks:
...
func _start_run(mark: Dictionary) -> void:
    ...
    run_amber = 0
    _clear_menu()
    title_label.text = biome_data[0].name
    subtitle_label.text = "%s • Rootmark: %s" % [selected_weapon.name, selected_mark.name]
    ...
    player.hp_changed.connect(func(_current, _maximum): _refresh_info())
...
func _do_attack() -> void:
    ...
    for enemy in enemies.duplicate():
        ...
        if enemy.position.distance_to(attack.center) <= attack.radius + enemy.radius:
            var force := attack.direction * float(attack.knockback)
            var dealt := enemy.take_damage(int(attack.damage), force)
            player.heal_from_damage(dealt)
            hit_any = true
    if hit_any: _start_shake(0.09, 1.5)
...
func _start_shake(duration: float, strength: float) -> void:
    if not bool(SaveManager.settings.get("screen_shake", true)): return
...
func _advance_encounter() -> void:
    if wave % 4 == 0:
        ...
        title_label.text = biome_data[depth - 1].name
...
func _show_settings() -> void:
    if state != STATE_SETTINGS: settings_return_state = state
    ...
    _button("REDUCE FLASHES: %s" % ("ON" if SaveManager.settings.reduce_flashes else "OFF"), _toggle_flashes)
    _button("BACK", _leave_settings)

func _leave_settings() -> void:
    if settings_return_state == STATE_HUB: _show_hub()
    else: _show_title()

func _cycle_volume() -> void:
    var value := float(SaveManager.settings.master_volume) - 0.2
...
func _toggle_flashes() -> void:
    SaveManager.settings.reduce_flashes = not bool(SaveManager.settings.reduce_flashes)
    SaveManager.save_settings()
    _show_settings()

func _refresh_info() -> void:
    if state != STATE_RUN or not is_instance_valid(player): return
...
func _draw() -> void:
    var background := Color("0e1714") if not high_contrast else Color.BLACK
    draw_rect(Rect2(0, 0, 320, 180), background)
    if state == STATE_RUN:
        var biome: Dictionary = biome_data[clampi(depth - 1, 0, biome_data.size() - 1)]
        var floor_color: Color = biome.floor if not high_contrast else Color("101010")
        var accent_color: Color = biome.accent if not high_contrast else Color("3b3b3b")
        draw_rect(Rect2(10, 24, 300, 144), floor_color.darkened(0.25))
        draw_rect(Rect2(12, 26, 296, 140), floor_color)
        for x in range(18, 306, 24): draw_rect(Rect2(x, 30 + ((x * 7 + depth * 17) % 120), 2, 5), accent_color)
