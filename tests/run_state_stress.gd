...
func _run() -> void:
    await process_frame
    original_save = SaveManager.save_data.duplicate(true)
    ...
    var wins_before := int(SaveManager.save_data.get("wins", 0))
    for cycle: int in range(15):
        ...
        _check(int(SaveManager.save_data.get("wins", 0)) == wins_before + cycle + 1, "clear cycle %d increments wins exactly once" % cycle)
...
func _finish() -> void:
    paused = false
    SaveManager.save_data = original_save.duplicate(true) if not original_save.is_empty() else SaveManager.save_data
